            ...
            <div class="box-body">
              ...
              <table id="example1" class="table table-bordered">
                ...
                <tbody>
                  <?php
                  $ct=0;
                   $sql = "SELECT *, voters.firstname AS first, voters.lastname AS last, voters.firstname AS votfirst, voters.matric AS votmatric, voters.fibsupics AS fibsupics, voters.feepics AS feepics, voters.receiptno AS receiptno,  voters.id AS id, voters.status AS status FROM votes LEFT JOIN positions ON positions.id=votes.position_id LEFT JOIN candidates ON candidates.id=votes.candidate_id LEFT JOIN voters ON voters.id=votes.voters_id WHERE votes.status='0'";
                    $query = $conn->query($sql);
                    while($row = $query->fetch_assoc()){
                        $dir=$row['last'].'-'.$row['first'].$row['votmatric']."/";
                        $status=$row['status'];
